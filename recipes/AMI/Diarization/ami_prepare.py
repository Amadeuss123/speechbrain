...
def prepare_ami(
    ...
    # Meta files
    # meta files 中为不同record中不同speaker的语音片段信息，并且对于过长的一段分为了最长为 max_subseg_dur的子段
    meta_files = [
    ...
    conf_file = "opt_ami_prepare." + mic_type + ".pkl"

    # Check if this phase is already done (if so, skip it)
    if skip(save_folder, conf, meta_files, conf_file):
        logger.info(
    ...
    dataset_split = {"train": train_set, "dev": dev_set, "eval": eval_set}
    for type, record_set in dataset_split.items():
        rttm_file = ref_rttm_dir + "/fullref_ami_" + type + ".rttm"
        prepare_segments_for_RTTM(
            record_set,
            rttm_file,
            data_folder,
            manual_annot_folder,
    ...
    for set_type in dataset_split.keys():
        rttm_file = ref_rttm_dir + "/fullref_ami_" + set_type + ".rttm"
        meta_filename_prefix = "ami_" + set_type
        prepare_metadata(
            rttm_file,
            meta_data_dir,
            data_folder,
    ...
    save_opt_file = os.path.join(save_folder, conf_file)
    save_pkl(conf, save_opt_file)


def get_RTTM_per_record(segments, speakers_list, record_id):
...
def prepare_segments_for_RTTM(
    record_id_list, out_rttm_file, audio_dir, annot_dir, split_type, skip_TNO
):
    ...
    for record_id in record_id_list:
        ...
        audio_file_name = audio_dir + "/" + record_id + "*"
        record_file_list = glob.glob(audio_file_name)
        record_file_list.sort()

        for record_file in record_file_list:
            record_id = os.path.basename(record_file)
            path = annot_dir + "/segments/" + record_id
            f = path + ".*.segments.xml"
            speaker_xmls_list = glob.glob(f)
            speaker_xmls_list.sort()  # A, B, C, D, E etc (Speakers)
            segs = []
            speakers_list = (
                []
            )  # Since non-scenario recordings contains 3-5 speakers

            for speaker_xml_file in speaker_xmls_list:
                # Speaker ID: A、B、C、D、E
                speaker = os.path.basename(speaker_xml_file).split(".")[1]
                speaker_ID = record_id + "." + speaker
                speakers_list.append(speaker_ID)

                # Parse xml tree
                tree = et.parse(speaker_xml_file)
                root = tree.getroot()
            ...
            rttm_per_rec = get_RTTM_per_record(segs, speakers_list, record_id)
...
def merge_overlap_segments(rttm_segments):
    """Merges adjacent segments in rttm if they overlap.
    """
    # For one recording
    # rec_id = rttm_segs[0][1]
    rttm_segments.sort(key=lambda x: float(x[3]))

    # first_seg = rttm_segs[0] # first interval.. as it is
    merged_segments = [rttm_segments[0]]
    start = float(rttm_segments[0][3])
    end = float(rttm_segments[0][3]) + float(rttm_segments[0][4])

    for row in rttm_segments[1:]:
        s = float(row[3])
        ...
        if is_overlapped(end, s):
            ...
            merged_segments[-1][3] = str(round(start, 4))
            merged_segments[-1][4] = str(round((end - start), 4))
            merged_segments[-1][7] = "overlap"  # previous_row[7] + '-'+ row[7]
        else:
            # Add a new disjoint segment
            start = s
            end = e
            merged_segments.append(row)  # this will have 1 spkr ID

    return merged_segments


def get_subsegments(merged_segments, max_subsegment_duration=3.0, overlap=1.5):
    """Divides bigger segments into smaller sub-segments
    """

    shift = max_subsegment_duration - overlap
    subsegments = []

    # These rows are in RTTM format
    for row in merged_segments:
        segment_duration = float(row[4])
        record_id = row[1]

        if segment_duration > max_subsegment_duration:
            subsegments_num = int(segment_duration / shift)
            # Taking 0.01 sec as small step
            segment_start = float(row[3])
            segment_end = segment_start + segment_duration

            # Now divide this segment (new_row) in smaller subsegments
            for i in range(subsegments_num):
                subsegment_start = segment_start + i * shift
                subsegment_end = min(subsegment_start + max_subsegment_duration - 0.01, segment_end)
                subsegment_duration = subsegment_end - subsegment_start

                new_row = [
                    "SPEAKER",
                    record_id,
                    "0",
                    str(round(float(subsegment_start), 4)),
                    str(round(float(subsegment_duration), 4)),
                    "<NA>",
                    "<NA>",
                    row[7],
                    "<NA>",
...
def prepare_metadata(
    rttm_file, save_dir, data_dir, filename, max_subsegment_duration, overlap, mic_type
):
    ...
    speaker_info = filter(lambda x: x.startswith("SPKR-INFO"), RTTM)
    rec_ids = list(set([row.split(" ")[1] for row in speaker_info]))
    ...
    for rec_id in rec_ids:
        ...
        split_rttm_segs = [row.split(" ") for row in segs_iter]

        # Merge, subsegment and then convert to json format.
        merged_segments = merge_overlap_segments(
            split_rttm_segs
        )  # We lose speaker_ID after merging
        MERGED_SEGMENTS = MERGED_SEGMENTS + merged_segments

        # Divide segments into smaller sub-segments
        subsegs = get_subsegments(merged_segments, max_subsegment_duration, overlap)
